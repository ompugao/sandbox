...
void main1(const std::shared_ptr<Parameters>& params) {
  ...
  Eigen::MatrixXd xref = Eigen::MatrixXd::Zero(vehicle.num_state_, params->horizon + 1);
...
void main2(const std::shared_ptr<Parameters>& params) {
  Vehicle vehicle(params);
  vehicle.set_state(0, 1, M_PI / 2.0, 0, 0, 0);
...
int main(int argc, char* argv[]) {
  ...
  LOG(INFO) << "Available SIMD Instructions: " << Eigen::SimdInstructionSetsInUse();
